...
def _build_surrogate(
    ...
    if backend == "bnn":
        from emb.indentation.surrogate.evaluate_bnn import Surrogate
...
def compute_indentation(  # pragma: no cover
    ...
    *,
    ...
    source_indentation_path = repo_root / "emb" / "indentation" / "src"

    if init_indentation_path is None:
        if project_root is None or diameter_um is None:
            init_indentation_path = "init_emb/indentation/"
        else:
