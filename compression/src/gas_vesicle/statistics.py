...
vertices, T = read_off_file("out.of")
...
for f in faces:
    loc_cms = (
        f[0] + f[1] + f[2]
    ) / 3.0 - cms  # triangle cms #loc_cms actually does not matter, as long as it is one of the vectors
...
print(
    f"""Average triangle area = {np.mean(area)} +- {np.std(area)}.
