...
int main (int argc, char* argv[])
{
  // Points with normals.
  Pwn_vector points;
  const std::string fname = (argc>1) ? argv[1] : CGAL::data_file_path("test1.xyz");
  ...
  double p4 = strtof(argv[4],NULL);
  int p5 = atoi(argv[5]);
  double p6 = strtof(argv[6],NULL);
  op.probability = p4; //0.05
  op.min_points = p5; //100
  op.epsilon = (argc>2 ? boost::lexical_cast<double>(argv[2]) : 0.002);
  op.cluster_epsilon = (argc>3 ? boost::lexical_cast<double>(argv[3]) : 0.02);
  op.normal_threshold = p6;//0.7;
