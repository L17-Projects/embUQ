    ...
    double *subProblemLogLikelihoods)
{
  ...
  __shared__ int sharedParametersAreFeasible;
  if (tid == 0)
  {
    bool parametersAreFeasible = true;
    for (unsigned int priorId = 0; priorId < dynamicPriorCount; ++priorId)
    {
      ...
      if (priorKinds[priorId] == 0)
      {
        if (parameterB <= 0.0)
        {
          parametersAreFeasible = false;
          break;
        }
      }
      else if (parameterB <= parameterA)
      {
        parametersAreFeasible = false;
        break;
      }
    }
    sharedParametersAreFeasible = parametersAreFeasible ? 1 : 0;
  }
  __syncthreads();

  if (sharedParametersAreFeasible == 0)
  {
    if (tid == 0)
      subProblemLogLikelihoods[(size_t)subProblemId * batchSize + candidateId] = koraliNegativeInfinity();
    return;
  }
  ...
  for (unsigned int sampleId = tid; sampleId < sampleCount; sampleId += blockDim.x)
  {
    ...
    for (unsigned int priorId = 0; priorId < dynamicPriorCount; ++priorId)
    {
      ...
      {
        if (sampleValue < parameterA || sampleValue > parameterB)
        {
...
void Psi::releaseNativeCudaBatch()
{
  ...
  if (_nativeCudaBatchParametersDevice != 0) KORALI_CUDA_DRIVER_CHECK(cuMemFree(_nativeCudaBatchParametersDevice));
  if (_nativeCudaSubProblemLogLikelihoodDevice != 0) KORALI_CUDA_DRIVER_CHECK(cuMemFree(_nativeCudaSubProblemLogLikelihoodDevice));
  ...
  _nativeCudaBatchParametersDevice = 0;
  _nativeCudaBatchParametersCapacityBytes = 0;
  _nativeCudaSubProblemLogLikelihoodDevice = 0;
  _nativeCudaSubProblemLogLikelihoodCapacityBytes = 0;
...
void Psi::evaluateBatchNativeCuda(Sample &sample)
{
  ...
  const size_t batchParametersBytes = flattenedBatchParameters.size() * sizeof(double);
  const size_t subProblemLogLikelihoodBytes = static_cast<size_t>(batchSizeInt) * subProblemCount * sizeof(double);

  auto freeStart = steadyClock::now();
  if (_nativeCudaBatchParametersCapacityBytes < batchParametersBytes && _nativeCudaBatchParametersDevice != 0)
  {
    KORALI_CUDA_DRIVER_CHECK(cuMemFree(_nativeCudaBatchParametersDevice));
    _nativeCudaBatchParametersDevice = 0;
    _nativeCudaBatchParametersCapacityBytes = 0;
  }
  if (_nativeCudaSubProblemLogLikelihoodCapacityBytes < subProblemLogLikelihoodBytes && _nativeCudaSubProblemLogLikelihoodDevice != 0)
  {
    KORALI_CUDA_DRIVER_CHECK(cuMemFree(_nativeCudaSubProblemLogLikelihoodDevice));
    _nativeCudaSubProblemLogLikelihoodDevice = 0;
    _nativeCudaSubProblemLogLikelihoodCapacityBytes = 0;
  }
  ...
  auto allocStart = steadyClock::now();
  if (_nativeCudaBatchParametersCapacityBytes < batchParametersBytes)
  {
    KORALI_CUDA_DRIVER_CHECK(cuMemAlloc(&_nativeCudaBatchParametersDevice, batchParametersBytes));
    _nativeCudaBatchParametersCapacityBytes = batchParametersBytes;
  }
  if (_nativeCudaSubProblemLogLikelihoodCapacityBytes < subProblemLogLikelihoodBytes)
  {
    KORALI_CUDA_DRIVER_CHECK(cuMemAlloc(&_nativeCudaSubProblemLogLikelihoodDevice, subProblemLogLikelihoodBytes));
    _nativeCudaSubProblemLogLikelihoodCapacityBytes = subProblemLogLikelihoodBytes;
  }
  allocSeconds = elapsedSeconds(allocStart, steadyClock::now());

  auto h2dStart = steadyClock::now();
  KORALI_CUDA_DRIVER_CHECK(cuMemcpyHtoD(_nativeCudaBatchParametersDevice, flattenedBatchParameters.data(), batchParametersBytes));
  h2dSeconds = elapsedSeconds(h2dStart, steadyClock::now());

  void *kernelParameters[] = {
      &_nativeCudaBatchParametersDevice,
      &parameterCount,
      &batchSizeInt,
      &dynamicPriorCount,
      ...
      &subProblemCount,
      &_nativeCudaSubProblemLogLikelihoodDevice};
  ...
  KORALI_CUDA_DRIVER_CHECK(cuMemcpyDtoH(subProblemLogLikelihoods.data(), _nativeCudaSubProblemLogLikelihoodDevice, subProblemLogLikelihoods.size() * sizeof(double)));
