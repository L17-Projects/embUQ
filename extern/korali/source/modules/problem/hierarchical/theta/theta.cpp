...
void Theta::initialize()
{
  ...
  _k->_variables.clear();
  for (size_t i = 0; i < _subExperimentObject._variables.size(); i++) _k->_variables.push_back(_subExperimentObject._variables[i]);
  _k->_distributions.clear();
  for (size_t i = 0; i < _subExperimentObject._distributions.size(); i++) _k->_distributions.push_back(_subExperimentObject._distributions[i]);
...
void Theta::evaluateBatch(Sample &sample)
{
  ...
  {
    if (batchParameters[i].size() != _k->_variables.size()) KORALI_LOG_ERROR("Batch sample %zu provides %zu parameters, but the hierarchical problem expects %zu.\n", i, batchParameters[i].size(), _k->_variables.size());
    double logPrior = 0.0;
  ...
  auto *subProblem = dynamic_cast<problem::Bayesian *>(_subExperimentObject._problem);
  if (subProblem == NULL || subProblem->supportsEvaluateBatch() == false) KORALI_LOG_ERROR("Hierarchical/Theta batch evaluation requires the sub problem to expose batch evaluation support.\n");
  Sample subBatch;
  ...
  try { subBatch["Batch Sample Ids"] = sample["Batch Sample Ids"]; } catch (...) {}
  ...
  if (subBatchLogLikelihood.size() != batchSize) KORALI_LOG_ERROR("Hierarchical/Theta batch evaluation received %zu subproblem log-likelihood values, expected %zu.\n", subBatchLogLikelihood.size(), batchSize);
...
void Theta::setConfiguration(knlohmann::json& js)
{
 ...
 if (isDefined(js, "Sub Experiment")) { _subExperiment = js["Sub Experiment"].get<knlohmann::json>(); eraseValue(js, "Sub Experiment"); } else KORALI_LOG_ERROR(" + No value provided for mandatory setting: ['Sub Experiment'] required by theta.\n");
 if (isDefined(js, "Psi Experiment")) { _psiExperiment = js["Psi Experiment"].get<knlohmann::json>(); eraseValue(js, "Psi Experiment"); } else KORALI_LOG_ERROR(" + No value provided for mandatory setting: ['Psi Experiment'] required by theta.\n");
...
void Theta::applyModuleDefaults(knlohmann::json& js) { Hierarchical::applyModuleDefaults(js); }
void Theta::applyVariableDefaults() { Hierarchical::applyVariableDefaults(); }
