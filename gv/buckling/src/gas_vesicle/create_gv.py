...
def make_off_file_for_ovito(filename,points):
    file = open(filename,'w')
    file.write(f'{int(len(points))}\n')
    file.write(f'Created by CGAL\n')
    for p in points:
        file.write(f'1 {p[0]} {p[1]} {p[2]}\n')
    file.close()
...
def gen_cone(radius, height, k, rho, sign = 1, shift = 0.5):
    ...
    for i in range(k - 1):
        ...
        npts = max(int(2.0 * rho * 2 * np.pi * radtmp), 3)
...
k0 = int(80 / 14.28 * height0)   #40/14.28 empirical factor that works well
...
make_off_file_for_ovito('test.xyz', arr)
