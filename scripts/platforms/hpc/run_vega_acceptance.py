#!/usr/bin/env python3
"""
Run the site-selected HPC acceptance command for MesoUQ.

This is intentionally a thin wrapper around the richer shared
`scripts/platforms/hpc/run_validation_suite.py` operator runner. It captures
environment metadata, invokes the validation suite once, and writes one
...
VALIDATION_RUNNER = PROJECT_ROOT / "scripts" / "platforms" / "hpc" / "run_validation_suite.py"
DEFAULT_WORKFLOWS = [
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the site-selected MesoUQ HPC acceptance command.")
    ...
    report_path = output_root / f"{resolved_site}_acceptance_report.json"
    report_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
    print(f"Acceptance report: {report_path}")
    if report["status"] == "failed":
        print(f"{resolved_site.capitalize()} acceptance failed. See logs and report for details.")
        return 1
    print(f"{resolved_site.capitalize()} acceptance status: {report['status']}")
    return 0
