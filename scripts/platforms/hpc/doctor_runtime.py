#!/usr/bin/env python3
"""Inspect a MesoUQ HPC environment for fresh-clone workflow readiness."""
...
def _path_is_relative_to(path: str | Path, parent: str | Path) -> bool:
...
def collect_diagnostics(
    ...
    *,
    ...
    if not with_gv_runtime:
        ...
        if korali_origin:
            status = "ok" if _path_is_relative_to(korali_origin, paths.korali_site_packages) else "warn"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Inspect the local HPC bootstrap/runtime surface.")
    ...
    try:
        site = resolve_hpc_site(cli_site=args.site, default="vega")
