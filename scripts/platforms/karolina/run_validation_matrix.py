...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the public validation matrix across the shipped validation workflows."
    )
    parser.add_argument("--selection", action="append", default=[], help="Explicit selection in experiment:model-family:validation form.")
    parser.add_argument("--experiments", nargs="+", choices=("compression", "indentation"), default=None)
    parser.add_argument("--model-families", nargs="+", choices=("full-model", "reduced-model"), default=None)
    ...
    command = [args.python_bin, str(SCRIPT_DIR.parent / "vega" / "run_workflow_matrix.py")]
