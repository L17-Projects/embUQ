...
| Mirheo source staging reproducibility | `src/meso_uq/vega.py`; `scripts/platforms/vega/bootstrap_mirheo.sh`; `scripts/platforms/karolina/bootstrap_mirheo.sh`; `src/meso_uq/site_runtime.py` | Medium | High | Keep lock/env/override precedence and source snapshots; enforce snapshot checks before runtime | Re-bootstrap Mirheo from explicit source or lock path; restore the snapshot; rerun the doctor and EMB acceptance tests | `bash scripts/platforms/vega/bootstrap_mirheo.sh --source <path> --python-bin $(command -v python)`; `python scripts/platforms/vega/doctor_vega.py --with-mirheo --strict`; `python -m pytest tests/test_vega_acceptance_smoke.py tests/test_karolina_validation_matrix.py` | Phase 2 / Phase 3 | Approve removal timing for old source compatibility bridges |
...
| Artifact movement and owner-approved deletion | `docs/ARTIFACT_POLICY.md`; `docs/MESO_UQ_MIGRATION_INVENTORY.md`; `src/meso_uq/artifacts/policy.py`; `tests/unit/test_artifact_manifest_policy.py` | Medium | High | Protect vendor/runtime roots; require owner-approved cleanup list, manifest/checksum evidence, and post-action GPU validation | Recreate deleted artifacts through bootstrap/regeneration scripts; restore from archive if validation fails | `python -m pytest tests/unit/test_artifact_manifest_policy.py`; GPU validation matrix after cleanup | Phase 3 / Phase 6 | Approve deletion inventory, external storage target, retention window, and rollback archive |
