...
|---|---|---|---|---|---|---|---|
| `compression.*` imports | 9 refs / 8 files, mostly tests and evaluation runtime coverage | No complete replacement yet; route future reusable calls to package APIs under `meso_uq` and shared EMB extraction modules | Legacy EMB modality boundary, active | Keep tolerated import compatibility while runtime extraction proceeds | Add deprecation warning only when a canonical package API exists, not before | One full release after canonical APIs and wrapper tests land | Medium |
| `indentation.*` imports | 9 refs / 6 files, mostly tests and evalkit runtime coverage | Same as compression | Legacy EMB modality boundary, active | Same as compression | Same as compression | Same as compression | Medium |
