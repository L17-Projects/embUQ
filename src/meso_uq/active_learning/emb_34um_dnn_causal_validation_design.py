...
def _sample_points(*, start_index: int, count: int) -> tuple[tuple[float, float], ...]:
    ...
    for step in range(count):
        x, y = _index_to_unit_point(start_index + step)
        values.append(
            (
                _from_unit_to_log_space(x, EMB_34UM_DNN_CAUSAL_BOUNDS["ka"]),
                _from_unit_to_log_space(y, EMB_34UM_DNN_CAUSAL_BOUNDS["kb"]),
            )
        )
    return tuple(values)
...
def _candidate_records(
    *,
    ...
    points = _sample_points(start_index=start_index, count=count)
...
def build_emb_34um_dnn_causal_validation_campaign_manifest(
    *,
    ...
    replica_count: int = EMB_34UM_DNN_CAUSAL_REPLICATE_COUNT,
    ...
    replica_count = _coerce_replica_count(replica_count)
    ...
    for replica in EMB_34UM_DNN_CAUSAL_REPLICATES[:replica_count]:
